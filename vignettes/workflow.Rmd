...
```{r}
data("contigs")
Ucounts <- clonoStats(contigs, method = 'unique')
class(Ucounts)
```
...
```{r}
EMcounts <- clonoStats(contigs, method = "EM")
class(EMcounts)
```
...
```{r}
x <- clonoStats(x, method = 'EM')
sampleCounts <- metadata(x)$clonoStats$abundance
head(sampleCounts)
```
...
```{r}
div <- calculateDiversity(metadata(x)$clonoStats, methods = "all")
div
```
