...
This barplot shows the number of cells in each group (sample) as well as the
clonotype abundances within each group. The coloring indicates the number of
cells assigned to each clonotype, with darker colors denoting singletons and
lighter colors denoting expanded clonotypes.
...
within each group (sample), as a percentage of cells within that group. Coloring
...
## Clonotype Abundance Scatter Plot

This dot plot shows the clonotype abundances in each group (sample) above a
specified cutoff. The most abundant clonotypes for each group are annotated on
